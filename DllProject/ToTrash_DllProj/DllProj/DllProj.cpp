...
LPARAM OnMyButton(const HWND& hWnd)
{
	...
	NoArgs(3); //implicit linking

	strcpy(mess, "Recieved message");
	Class cl; cl.s = mess; cl.b = mask; cl.c = strlen(mess); cl.r = 5.9;
	int n = SymbolCount(mess, mask, strlen(mess), 5.9);   //implicit linking
	wsprintf(info, _T("A number of entries of symbol '%c' in string = %d"), mask, n);
